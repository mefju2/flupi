<script lang="ts">
  ...
  function openContextMenu(e: MouseEvent, node: RequestTreeNode) {
    const items = buildContextMenuItems(node,
      ...
      (id) => { pendingInput = { type: 'rename-request', id, value: node.name }; },
      (id) => { pendingDelete = { type: 'request', id, label: node.name }; },
      async (id) => { if (!$project.path) return; try { await duplicateRequest($project.path, id); await reload(); } catch (e) { console.error(e); } },
      (folder) => { pendingInput = { type: 'rename-collection', id: folder, value: node.name }; },
      (folder) => { pendingDelete = { type: 'collection', id: folder, label: node.name }; }
    );
...
<div class="flex flex-col h-full bg-app-panel">
    ...
    {#each collections as collection (collection.folder_name)}
      ...
      >
        {#each items as item (item.id)}
          <TreeNode node={item.node} activeRequestId={$activeRequestId} onSelect={selectRequest} onContextMenu={openContextMenu} showDragHandle={true} inlineEdit={makeInlineEdit(item)} />
        {/each}
  ...
  {#if pendingDelete}
    <div class="px-3 py-2 border-t border-app-border bg-app-card">
      <p class="text-xs text-app-text-2 mb-2">Delete <span class="text-app-text font-mono">{pendingDelete.label}</span>?</p>
      <div class="flex gap-2">
        <button class="text-xs text-red-400 hover:text-red-300 transition-colors" onclick={confirmDelete}>Delete</button>
        <button class="text-xs text-app-text-3 hover:text-app-text-2 transition-colors" onclick={() => pendingDelete = null}>Cancel</button>
      </div>
    </div>
  {/if}
