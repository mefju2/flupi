  ...
  >
    ...
    <div class="flex items-center gap-2 shrink-0">
      {#if statusCode !== undefined}
        <StatusBadge
          variant={statusCode >= 200 && statusCode < 300 ? "success" : "error"}
        >
          {statusCode}
        </StatusBadge>
      {/if}
      ...
      {#if extractedEntries.length > 0}
        ...
        {#if extractedEntries.length > 3}
          <span class="text-xs text-app-text-3"
            >+{extractedEntries.length - 3} more</span
          >
        {/if}
      {/if}
    ...
    <div class="border-t border-app-border px-3 py-3 space-y-3">
      {#if statusCode !== undefined || duration !== undefined}
        <div class="flex items-center gap-2">
          {#if statusCode !== undefined}
            <StatusBadge
              variant={statusCode >= 200 && statusCode < 300
                ? "success"
                : "error"}>{statusCode}</StatusBadge
            >
          {/if}
          {#if duration !== undefined}
            <span class="text-xs text-app-text-3">{duration}ms</span>
      ...
      {#if result.error}
        <div>
          ...
          <pre
            class="font-mono text-xs text-red-400 bg-app-card rounded p-2 overflow-x-auto">{result.error}</pre>
        </div>
      {/if}
          ...
          <div class="space-y-1">
            {#each extractedEntries as [key, value]}
              ...
              <div class="flex gap-2 items-start min-w-0">
                <span
                  class="font-mono text-xs text-app-text-3 min-w-24 shrink-0 pt-0.5"
                  >{key}</span
                >
                {#if isJson}
                  <div class="flex-1 min-w-0">
                    {#if expandedExtracted.has(key)}
                      <div class="flex items-start gap-2">
                        <pre
                          class="font-mono text-xs text-app-text-2 bg-app-card rounded p-2 overflow-auto max-h-64 flex-1 whitespace-pre-wrap break-all min-w-0">{JSON.stringify(
                            parsedVal,
                            null,
                            2,
                          )}</pre>
                        <button
                          ...
                          onclick={() => toggleExtracted(key)}>Collapse</button
                        >
                      </div>
                    {:else}
                      <button
                ...
                {:else}
                  <span class="font-mono text-xs text-app-text truncate min-w-0"
                    >{value}</span
                  >
                {/if}
              </div>
      ...
      {#if result.sent_request}
        <SentRequestCard sentRequest={result.sent_request} />
      {/if}
      ...
      {#if result.response}
        ...
        <div>
          ...
          {#if bodyExpanded}
            <pre
              class="font-mono text-xs text-app-text-2 bg-app-card rounded p-2 overflow-auto max-h-96 whitespace-pre-wrap break-all">{pretty}</pre>
          {:else}
            <button
