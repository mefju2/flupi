<script lang="ts">
  ...
  onMount(() => {
    const onRunShortcut = () => {
      if ($activeScenario) handleRunGuard();
    };
...
<div class="flex h-full">
  ...
  <ResizablePanel
    ...
    maxWidth={420}
    side="right"
    storageKey="flupi:sidebar-scenarios"
  >
  ...
  <div class="flex-1 flex flex-col min-w-0 relative">
    {#if saveToast}
      <div
        ...
        class="absolute top-2 right-17.5 z-10 text-xs text-cyan-400 bg-app-card border border-app-border-2 rounded px-3 py-1"
      >
        {saveToast}
      </div>
    {/if}
